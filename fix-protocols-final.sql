-- Final fix for protocols in mood snapshots
-- This will ensure protocols include name and frequency

-- Step 1: Check what columns actually exist in protocols table
SELECT column_name, data_type 
FROM information_schema.columns 
WHERE table_name = 'protocols' 
ORDER BY ordinal_position;

-- Step 2: Update the upsert function to include protocol names
...
BEGIN
  ...
  -- Include name and frequency if they exist
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'id', pr.id, 
    'name', pr.name,
    'phase', 'active', 
    'notes', COALESCE(pr.frequency, '')
...
-- Step 3: Update existing entries with proper protocol data
DO $$
DECLARE
  entry_record RECORD;
  v_meds jsonb;
...
BEGIN
  ...
  LOOP
    ...
    -- Get protocols for this user on this date with names
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'id', pr.id, 
      'name', pr.name,
      'phase', 'active', 
