...
CREATE TEMP TABLE _target_slugs ON COMMIT DROP AS
SELECT unnest(ARRAY['donotage-suresleep', 'seeking-health-optimal-focus']::text[]) AS slug;

CREATE TEMP TABLE _target_cohort_ids ON COMMIT DROP AS
SELECT c.id
FROM public.cohorts c
WHERE c.slug IN (SELECT slug FROM _target_slugs);
...
JOIN public.cohorts c ON c.id = cp.cohort_id
LEFT JOIN public.profiles p ON p.id = cp.user_id
WHERE NOT EXISTS (SELECT 1 FROM _target_slugs ts WHERE ts.slug = c.slug);
...
DO $$
BEGIN
  IF to_regclass('public.study_waitlist') IS NOT NULL THEN
    DELETE FROM public.study_waitlist WHERE cohort_id IN (SELECT id FROM _target_cohort_ids);
  END IF;
END $$;
...
WHERE pr.user_id IN (
  ...
  AND pr.cohort_id IN (SELECT slug FROM _target_slugs);

UPDATE public.profiles pr
SET cohort_id = NULL
WHERE pr.cohort_id IN (SELECT slug FROM _target_slugs)
  AND pr.user_id NOT IN (SELECT auth_id FROM _auth_remove_entirely)
  AND NOT EXISTS (
    SELECT 1 FROM public.cohort_participants cp WHERE cp.user_id = pr.id
  );
