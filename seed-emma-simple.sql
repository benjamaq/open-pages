-- Demo Account: Emma - Chronic Pain Management
-- Simplified version that works with standard Supabase setup
...
DELETE FROM profiles WHERE slug IN (
  'emma-chronic-pain-journey', 'jessica-wellness', 'family-support', 
  'sarah-similar-journey', 'dr-chen-pain-specialist'
);
DELETE FROM auth.users WHERE email IN (
  'emma.demo@biostackr.io', 'jessica.demo@biostackr.io', 
  'mom.demo@biostackr.io', 'sarah.demo@biostackr.io', 'drchen.demo@biostackr.io'
);

-- Now create the demo account
DO $$
DECLARE
  emma_user_id UUID;
  ...
  pain_flare BOOLEAN;
BEGIN
  -- Create auth user for Emma (using pgcrypto extension for password hashing)
  INSERT INTO auth.users (
    ...
    '$2a$10$dummyencryptedpasswordhash1234567890123456789012', -- Dummy hash, won't be used for login
  ...
  RAISE NOTICE 'Created % stack items for Emma', 13;
  ...
  FOR i IN 0..89 LOOP
    ...
    INSERT INTO daily_entries (
      user_id, local_date, mood, sleep_quality, pain, sleep_hours, night_wakes, tags, journal, created_at
    ) VALUES (
      emma_user_id,
      entry_date,
      mood_score,
  ...
  -- Create follower auth users (simple version)
  ...
  (follower_1_id, '00000000-0000-0000-0000-000000000000', 'jessica.demo@biostackr.io', '$2a$10$dummyencryptedpasswordhash1234567890123456789012', NOW() - INTERVAL '35 days', NOW() - INTERVAL '35 days', NOW(), '{"provider":"email","providers":["email"]}', '{"display_name":"Jessica"}', false, 'authenticated', 'authenticated'),
  (follower_2_id, '00000000-0000-0000-0000-000000000000', 'mom.demo@biostackr.io', '$2a$10$dummyencryptedpasswordhash1234567890123456789012', NOW() - INTERVAL '30 days', NOW() - INTERVAL '30 days', NOW(), '{"provider":"email","providers":["email"]}', '{"display_name":"Mom"}', false, 'authenticated', 'authenticated'),
  (follower_3_id, '00000000-0000-0000-0000-000000000000', 'sarah.demo@biostackr.io', '$2a$10$dummyencryptedpasswordhash1234567890123456789012', NOW() - INTERVAL '25 days', NOW() - INTERVAL '25 days', NOW(), '{"provider":"email","providers":["email"]}', '{"display_name":"Sarah"}', false, 'authenticated', 'authenticated'),
  (follower_4_id, '00000000-0000-0000-0000-000000000000', 'drchen.demo@biostackr.io', '$2a$10$dummyencryptedpasswordhash1234567890123456789012', NOW() - INTERVAL '20 days', NOW() - INTERVAL '20 days', NOW(), '{"provider":"email","providers":["email"]}', '{"display_name":"Dr. Chen"}', false, 'authenticated', 'authenticated');
  ...
  RAISE NOTICE 'Note: Login disabled for demo accounts (dummy password hash)';
