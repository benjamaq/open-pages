-- NUCLEAR OPTION: Drop all versions by selecting from pg_proc
-- This will definitely remove all conflicting function versions

DO $$ 
DECLARE
    func_signature text;
BEGIN
    -- Loop through all versions of upsert_daily_entry_and_snapshot and drop them
    ...
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS upsert_daily_entry_and_snapshot CASCADE';
        EXIT; -- Exit after first drop since CASCADE will remove all
    END LOOP;
END $$;

-- Add missing columns to daily_entries table (if they don't exist)
...
-- Create the ONE TRUE function version
