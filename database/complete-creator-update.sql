...
CREATE POLICY IF NOT EXISTS "Users can view public gear" ON gear
    FOR SELECT USING (public = true);
    
CREATE POLICY IF NOT EXISTS "Users can view their own gear" ON gear
    FOR SELECT USING (
        EXISTS (
...
CREATE POLICY IF NOT EXISTS "Users can insert gear for their own profiles" ON gear
...
CREATE POLICY IF NOT EXISTS "Users can update gear for their own profiles" ON gear
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = gear.profile_id 
...
CREATE POLICY IF NOT EXISTS "Users can delete gear for their own profiles" ON gear
    FOR DELETE USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = gear.profile_id 
...
CREATE POLICY IF NOT EXISTS "Users can view public shop gear items" ON shop_gear_items
    FOR SELECT USING (public = true);

CREATE POLICY IF NOT EXISTS "Users can view their own shop gear items" ON shop_gear_items
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM profiles 
...
CREATE POLICY IF NOT EXISTS "Users can insert shop gear items for their own profiles" ON shop_gear_items
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = shop_gear_items.profile_id 
...
CREATE POLICY IF NOT EXISTS "Users can update shop gear items for their own profiles" ON shop_gear_items
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = shop_gear_items.profile_id 
...
CREATE POLICY IF NOT EXISTS "Users can delete shop gear items for their own profiles" ON shop_gear_items
    FOR DELETE USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = shop_gear_items.profile_id 
