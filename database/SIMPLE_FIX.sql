-- Simple fix: Just update the RPC function signature to match what the app is sending
-- This assumes the columns already exist in daily_entries table
...
BEGIN
  ...
  -- Activity and devices are just empty for now
  v_activity := '[]'::jsonb;
  v_devices := '[]'::jsonb;
  ...
  -- Upsert the daily entry (WITHOUT energy column since it doesn't exist in your DB)
