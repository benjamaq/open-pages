-- URGENT SECURITY FIX - CRITICAL VULNERABILITIES
...
-- 5. ADD MISSING RLS POLICIES FOR NEWER TABLES

-- Add RLS for any missing tables
DO $$
BEGIN
  -- Check if tables exist and add RLS if missing
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'beta_codes' AND table_schema = 'public') THEN
    ALTER TABLE beta_codes ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'stack_followers' AND table_schema = 'public') THEN
    ALTER TABLE stack_followers ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'waitlist_signups' AND table_schema = 'public') THEN
    ALTER TABLE waitlist_signups ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'email_prefs' AND table_schema = 'public') THEN
...
FROM pg_tables 
WHERE tablename IN ('beta_codes', 'stack_followers', 'waitlist_signups', 'email_prefs', 'profiles', 'stack_items', 'protocols', 'uploads', 'user_usage')
