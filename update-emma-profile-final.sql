-- Update Emma's existing profile with complete data - FINAL VERSION
-- User ID: c1b5662e-73dd-48b1-a5d8-ec0d1a648415
-- This version avoids all foreign key constraint issues
...
DECLARE
    ...
    v_date DATE;
BEGIN
    RAISE NOTICE 'Updating Emma profile for user: %', v_emma_user_id;
    ...
    -- Update the existing profile with Emma's data
    UPDATE profiles SET
        slug = 'emma-chronic-pain-journey',
        display_name = 'Emma',
        bio = 'Chronic pain warrior sharing my journey with Low Dose Naltrexone (LDN) therapy. After 3 months of 8-9/10 pain, I found relief through systematic tracking and LDN. Documenting the good, the bad, and everything in between.',
    ...
    RAISE NOTICE 'Updated Emma profile data';
...
To anyone following this journey - thank you for your support. Chronic pain can be incredibly isolating, but knowing others are rooting for you makes all the difference.', true, '2025-10-10 14:00:00');
    ...
    -- ============================================
    -- LIBRARY ITEMS (with proper file_url)
    -- ============================================
    
    INSERT INTO library_items (profile_id, title, category, is_public, provider, summary_public, file_type, file_url, created_at) VALUES
    (v_emma_profile_id, 'LDN Prescription & Lab Results', 'lab', true, 'Dr. Sarah Chen, MD', 'Comprehensive lab panel showing inflammation markers and LDN prescription details.', 'application/pdf', 'https://example.com/ldn-prescription.pdf', '2025-09-08'),
    (v_emma_profile_id, 'Physical Therapy Assessment', 'assessment', true, 'Mike Rodriguez, DPT', 'Initial PT evaluation and treatment plan for chronic pain management.', 'application/pdf', 'https://example.com/pt-assessment.pdf', '2025-09-01'),
    (v_emma_profile_id, 'Pain Management Protocol', 'other', true, 'Dr. Sarah Chen, MD', 'Detailed treatment protocol including LDN titration schedule and monitoring plan.', 'application/pdf', 'https://example.com/pain-protocol.pdf', '2025-09-08');
    
    -- ============================================
    -- SKIP FOLLOWERS (to avoid foreign key constraints)
    -- ============================================
    
    RAISE NOTICE 'Skipping followers to avoid foreign key constraint issues';
    
    RAISE NOTICE 'Emma profile update COMPLETE!';
