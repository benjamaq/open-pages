...
export async function POST() {
  ...
  const successUrl = process.env.NEXT_PUBLIC_APP_URL ? `${process.env.NEXT_PUBLIC_APP_URL}/unlocked?member=1` : 'http://localhost:3010/unlocked?member=1'
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: [{
      price_data: {
        currency: 'usd',
        product_data: { name: 'BioStackr Membership' },
