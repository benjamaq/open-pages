-- Complete symptom tracking migration
-- Adds all required columns for the enhanced daily check-in

-- Add all symptom tracking columns if they don't exist
...
-- Update the RPC function to handle all new parameters
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'id', d.id, 'name', d.name, 'brand', d.brand, 'model', d.model, 'category', d.category
